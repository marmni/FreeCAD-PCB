    ...
    def getPads(self, layerNew, layerNumber, layerSide, tentedViasLimit, tentedVias):
        ...
        for i in self.projektBRD.getElementsByTagName("via"):
            ...
            if shape == "round":
                ...
                octagonName = "OCT_{0}_{1}_{2}".format(0, 0, diameter)
                
                if not octagonName in dummyObjects.keys():
                    layerNew.addOctagon(0, 0, diameter)
                    layerNew.setFace()
                    
                    dummyObjects[octagonName] = layerNew.spisObiektowTXT[-1]
                    
                    layerNew.addPlacement([x, y, 0], 0, [0, 0, 0])
                else:
                    try:
                        layerNew.addNewObject(dummyObjects[octagonName])
                        layerNew.addPlacement([x, y, 0], 0, [0, 0, 0])
                    except Exception as e:
                        print("{0}\n".format(e))
                    else:
                        print("{0}".format(layerNew.spisObiektowTXT[-1].Placement))
        #####
        if not tentedVias:
            self.getLibraries()
            self.getElements()
