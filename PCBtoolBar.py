...
class pcbToolBar(pcbToolBarMain):
    def __init__(self, parent=None):
        ...
        # sections
        scriptCmd_crossSections_M = self.createAction(u"Cross sections", u"Create sections from selected models and export them as library", ":/data/img/boundingBoxAll.svg")
        QtCore.QObject.connect(scriptCmd_crossSections_M, QtCore.SIGNAL("triggered()"), self.createSectionsSelParts)
        scriptCmd_crossSections = self.createAction(u"Cross sections", u"Create sections from selected models and export them as library", ":/data/img/boundingBoxAll.svg")
        QtCore.QObject.connect(scriptCmd_crossSections, QtCore.SIGNAL("triggered()"), self.createSectionsSelParts)
        scriptCmd_crossSectionsPCB = self.createAction(u"Cross sections from whole PCB", u"Create sections from PCB board (with components) and export them as library", ":/data/img/boundingBoxSelected.svg")
