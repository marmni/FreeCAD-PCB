...
def createBoxForModel(elem):
    ...
    try:
        if "Shape" in elem.PropertiesList and not elem.Shape.isNull() and hasattr(elem, "Proxy") and hasattr(elem.Proxy, "Type") and not elem.Proxy.Type in ["ShapeString"]:
            BoundBox = FreeCAD.ActiveDocument.addObject("Part::Box", "PCB_Bounding_Box_1")
            BoundBox.Label = "PCB Bounding Box 1"
...
def boundingBox():
    return createBox([i.Shape for i in FreeCAD.activeDocument().Objects if i.ViewObject.Visibility and "Shape" in i.PropertiesList and not i.Shape.isNull()])
    
def boundingBoxFromSelection(parts):
    return createBox([i.Shape for i in parts if i.ViewObject.Visibility and "Shape" in i.PropertiesList and not i.Shape.isNull()])
